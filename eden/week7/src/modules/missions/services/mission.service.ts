...
import { AlreadyChallengingMission, InvalidMission, InvalidStore, InvalidUser, NO_CHALLENGING_MISSIONS } from "../../../common/errors/errors";
...
export const createMission = async (storeId: number, missionData: AddMissionRequestDTO) => {
  ...
  if (!store) {
    throw new InvalidStore("존재하지 않는 가게입니다.");
  }
...
export const listStoreMissions = async (storeId: number) => {
    ...
    if(missions==null){
        throw new InvalidMission("해당 가게에 미션이 존재하지 않습니다.");
    }
...
export const startMissionChallenge = async (userId: number, missionId: number) => {
  ...
  if (!user) {
    throw new InvalidUser("존재하지 않는 사용자입니다.");
  }

  // 미션 존재 여부 확인
  const mission = await getMission(missionId); 
  if (!mission) {
    throw new InvalidMission("존재하지 않는 미션입니다.");}
  //이미 진행중인지 확인
...
export const completeMission = async (userId: number, missionId: number) => {
  ...
  if (existCompleteChallenge && existCompleteChallenge.some(mission => mission.id === missionId)) {
    throw new InvalidMission("이미 완료된 미션입니다.");
  }
  // 업데이트
