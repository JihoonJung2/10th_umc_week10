...
import { ReviewNotExist, StoreNotExist, UserNotExist } from "../../../common/errors/errors.js";



// createReview 함수의 파라미터를 DTO 객체로 받도록 수정
export const createReview = async (storeId: number, userId: number, reviewData: AddReviewRequestDTO) => {
  const store = await getStoreById(storeId);
  if (!store) {
    throw new StoreNotExist("가게 정보가 유효하지 않습니다.");
  }
  if (!userId) {
...
export const listMyReviews = async (userId: number) => {
   ...
   for (const review of reviews) {
    if (getStoreById(review.store.id) === null) {
      throw new ReviewNotExist("리뷰에 연결된 가게 정보가 유효하지 않습니다.");
    }
  }
